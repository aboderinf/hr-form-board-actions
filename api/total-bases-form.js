...
async function fetchJson(url) {
  const response = await fetch(url, {
    headers: { Accept: 'application/json', 'User-Agent': 'MLBTotalBasesForm/1.0' },
    cache: 'no-store',
...
async function batterGameLog(batterId, season, slateDate) {
  const cacheKey = `mlbtb2:gamelog:${slateDate}:${batterId}`;
  try {
    const cached = await redisCommand(['GET', cacheKey]);
    if (cached) return JSON.parse(cached);
  } catch (_) {
    // The form API can still operate from MLB if the non-critical log cache misses.
  }
  const url = `${MLB}/people/${batterId}/stats?stats=gameLog&group=hitting&season=${season}`;
  const payload = await fetchJson(url);
  const rows = payload?.stats?.[0]?.splits || [];
  try {
    await redisCommand(['SET', cacheKey, JSON.stringify(rows), 'EX', 259200]);
  } catch (_) {
    // Cache write failure should not hide a valid live form board.
  }
  return rows;
}
...
function windowMetrics(starts, n, prior) {
  ...
  if (!sample.length) {
    return {
      games: 0, hits2Plus: 0, hitRate: null, adjustedHitRate: prior,
      avgTb: null, xbhRate: null, paPerGame: null,
    };
  }
...
function quoteSummary(odds) {
  ...
  for (const [book, sides] of Object.entries(odds || {})) {
    ...
    if (sides?.under && Number(sides.under.line) === TARGET_LINE) {
      quotes.push({ book, side: 'under', ...sides.under });
    }
  }
...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    const diagnostics = [];
    let cursor = 0;
    const workers = Array.from({ length: Math.min(12, uniqueBatters.length) }, async () => {
      while (cursor < uniqueBatters.length) {
        const index = cursor++;
        const batter = uniqueBatters[index];
        try {
          const log = await batterGameLog(Number(batter.batterId), season, date);
          histories.set(Number(batter.batterId), startsBefore(log, date));
        } catch (error) {
          histories.set(Number(batter.batterId), []);
          diagnostics.push(`${batter.batterName}: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
    });
    ...
    const output = {
      ...
      methodology: {
        ...
        history: 'Only games strictly before the slate date are used. When gamesStarted is provided by MLB, pinch-hit-only appearances are excluded.',
