...
class EdgeSourceTests(unittest.TestCase):
    def test_parses_only_persisted_confirmed_prices(self):
        ...
        dk = next(
            price for price in player["prices"] if price["book"] == "DraftKings"
        )
    ...
    def test_rejects_price_snapshot_after_checkpoint(self):
        payload = sample_payload()
        payload["generatedAt"] = "2026-08-02T15:18:00Z"
        with self.assertRaisesRegex(ValueError, "after the checkpoint"):
            parse_edge_payload(payload, date(2026, 8, 2))
