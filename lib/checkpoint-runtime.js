...
async function redisCommand(command, { raw = false } = {}) {
  ...
  const wireCommand = !raw && operation === 'SET'
    ? [command[0], command[1], encodeRedisValue(command[1], command[2]), ...command.slice(3)]
    : command;
  ...
  return !raw && operation === 'GET'
    ? decodeRedisValue(payload.result) ?? readColdArchive(command[1])
    : payload.result;
}
