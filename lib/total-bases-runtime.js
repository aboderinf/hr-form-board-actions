...
function parsePlayerIdentity(entity) {
  ...
  const nameParts = hasMlbSuffix ? parts.slice(0, -2) : parts;
  ...
  return {
    name,
    mlbamId: hasMlbSuffix ? Number(parts.at(-2)) : null,
  };
}
...
function normalizeTotalBasesProviderPayload(raw, slateDate, checkpoint, requestedAt, completedAt) {
  ...
  for (const event of raw?.data || []) {
    ...
    for (const [oddKey, oddValue] of Object.entries(event?.odds || {})) {
      ...
      const identity = parsePlayerIdentity(providerEntityId);
      if (!identity.name) continue;
      const side = String(odd.sideID || '').toLowerCase();
      const rowKey = `${eventId}:${providerEntityId || playerKey(identity.name)}`;

      if (!rows.has(rowKey)) {
        rows.set(rowKey, {
          gameDate: slateDate,
          gameStartAt,
          batterId: identity.mlbamId,
          batterName: identity.name,
          playerKey: playerKey(identity.name),
  ...
  return {
    ...
    identityMapping: 'provider-mlbam-id',
    books: BOOKS,
