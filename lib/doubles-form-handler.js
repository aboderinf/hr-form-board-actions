...
const { normalizeCheckpoint, playerKey, redisCommand, readTriplesCheckpoint } = require('./checkpoint-runtime');
...
function formOnlyPayload(triples) {
  return {
    ...triples,
    market: 'batter-doubles-form-universe',
    rows: (triples?.rows || []).map((row) => ({ ...row, odds: {} })),
  };
}

async function resolveCheckpoint(date, requested) {
  const checkpoints = requested
    ? [normalizeCheckpoint(requested)].filter(Boolean)
    : CHECKPOINTS;

  for (const cp of checkpoints) {
    const doubles = await readDoublesCheckpoint(date, cp);
    if (doubles?.status === 'ready') {
      return { checkpoint: cp, payload: doubles, priceStatus: 'ready', universeSource: 'doubles-archive' };
    }

    const triples = await readTriplesCheckpoint(date, cp);
    if (triples?.status === 'ready') {
      return {
        checkpoint: cp,
        payload: formOnlyPayload(triples),
        priceStatus: 'awaiting-doubles-archive',
        universeSource: 'shared-offered-hitter-universe',
      };
    }
  }
  return null;
  ...
  try {
    ...
    if (!resolved?.payload) {
      ...
      return response.status(404).json({
        ...
        message: 'No shared archived hitter checkpoint is available for this slate yet',
    ...
    for (const oddsRow of hydrated.rows) {
      ...
      if (resolved.priceStatus === 'ready' && !quote.bestYes) continue;
      rows.push({
    ...
    const output = {
      ...
      universeSource: resolved.universeSource,
      providerRequests: 0,
      quotaObjectsAdded: 0,
      methodology: {
        ...
        eligibility: resolved.priceStatus === 'ready'
          ? 'At least one double in the prior 15 plate-appearance games and a genuine current archived doubles quote.'
          : 'At least one double in the prior 15 plate-appearance games among hitters already present in the shared pregame offered-hitter archive.',
        history: 'Only games strictly before the slate date are used. MLB game logs are fetched in bulk.',
        pricePolicy: resolved.priceStatus === 'ready'
          ? 'DraftKings, FanDuel and BetMGM doubles prices are display/execution inputs only and never affect form rank.'
          : 'No doubles quote is available in this checkpoint. No home-run, triples, or other market price is substituted.',
      },
      coverage: {
