...
function tuneStrategy(rows) {
  ...
  return best || { minEdge: 0.04, minEv: 0.05, score: null, stats: strategySummary(rows.filter((row) => row.modelEdge >= 0.04 && row.modelEv >= 0.05)) };
}
...
module.exports = async function totalBasesModelHandler(request, response) {
  ...
  try {
    ...
    const selectRule = (rows) => rows.filter((row) => Number(row.modelEdge) >= strategy.minEdge && Number(row.modelEv) >= strategy.minEv);
    ...
    for (const oddsRow of currentHydrated) {
      ...
      liveRows.push({
        ...
        qualifies: Number(modelProbability - implied) >= strategy.minEdge && Number(ev) >= strategy.minEv,
    ...
    const output = {
      schemaVersion: 1,
      kind: 'batter_two_plus_total_bases_model_v1',
      ...
      validation: {
        ...
        strategyRule: { minProbabilityEdge: strategy.minEdge, minExpectedValue: strategy.minEv, tunedOn: 'calibration archive only' },
        calibrationStrategy,
        holdoutStrategy,
