...
function gradeSelection(selection, date, gameLogs) {
  const appearance = settledBatterGame(gameLogs.get(Number(selection.batterId)), date);
...
function summarizeDays(days) {
  ...
  return {
    ...
    nonStarterVoids: usable.reduce((s, d) => s + Number(d.nonStarterVoids || 0), 0),
    unverified: usable.reduce((s, d) => s + Number(d.unverified || 0), 0),
    hitRate: bets ? Number((wins / bets).toFixed(4)) : null,
    netUnits: Number(netUnits.toFixed(3)), roi: bets ? Number((netUnits / bets).toFixed(4)) : null,
...
async function forwardPerformance(through) {
  ...
  const daily = snapshots.map(({ date, snapshot }) => {
    ...
    return {
      date, status: summary.ambiguous > 0 || summary.unverified > 0 ? 'partial' : 'settled', ...summary,
  ...
  return {
    ...
    overallOutOfSample: {
      ...
      nonStarterVoids: allForward.nonStarterVoids, unverified: allForward.unverified,
  ...
  try {
    ...
    const output = {
      ...
      methodology: {
        ...
        forwardTracking: 'The checkpoint workflow writes daily selection snapshots. This public endpoint settles them from official MLB hitting and fielding game logs: a pregame hitter prop is graded only when the hitter started and recorded a plate appearance; otherwise it is void. Unverified starter status never becomes a loss.',
      },
