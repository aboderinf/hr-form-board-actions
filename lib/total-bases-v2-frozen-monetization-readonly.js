...
const SEEDED_SNAPSHOTS = Object.freeze({
  '2026-08-26': {
    schemaVersion: 1, kind: 'frozen_total_bases_selection_snapshot',
    date: '2026-08-26', checkpoint: '0817', source: 'original-live-endpoint-observation',
    capturedAt: '2026-08-26T00:10:42.266Z', rule: FROZEN_RULE,
    selections: [
      { batterId: 645277, batterName: 'Ozzie Albies', matchup: 'LAD @ ATL', odds: 171, book: 'draftkings', executionProbability: 0.3996, edge: 0.0306, ev: 0.0829 },
      { batterId: 545361, batterName: 'Mike Trout', matchup: 'CLE @ LAA', odds: 167, book: 'draftkings', executionProbability: 0.3988, edge: 0.0243, ev: 0.0649 },
      { batterId: 660688, batterName: 'Keibert Ruiz', matchup: 'COL @ WSH', odds: 145, book: 'betmgm', executionProbability: 0.4315, edge: 0.0234, ev: 0.0572 },
    ],
  },
});
...
async function readSelectionSnapshot(date) {
  ...
  return SEEDED_SNAPSHOTS[date] || null;
}
...
function snapshotSummary(snapshot) {
  if (!snapshot) return { status: 'missing', selections: null, capturedAt: null, source: null };
  return { status: 'saved', selections: Array.isArray(snapshot.selections) ? snapshot.selections.length : 0, capturedAt: snapshot.capturedAt || null, source: snapshot.source || null };
}
  ...
  try {
    ...
    const output = {
      ...
      methodology: {
        ...
        forwardTracking: 'The checkpoint workflow is the only writer of daily selection snapshots. This public endpoint only reads saved selections and grades them. The ledger always runs through the latest completed ET day, independent of the slate-date picker.',
      },
