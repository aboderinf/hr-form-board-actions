...
async function pruneStaleTriplesState({ keepVersions = 3 } = {}) {
  ...
  const active = audit.activeTriplesStateVersion;
  const ordered = (audit.triplesStateVersions || []).map((row) => row.version);
  ...
  let keysDeleted = 0;
  let bytesFreed = 0;
  for (const row of audit.triplesStateVersions || []) {
    if (keep.has(row.version)) continue;
    const pattern = `mlbhr:triples-model:state:${row.version}:part-*.bin`;
    let cursor = '0';
    let versionKeys = [];
    do {
      const page = await redisCommand(['SCAN', cursor, 'MATCH', pattern, 'COUNT', 100]);
      cursor = String(page[0]);
      versionKeys.push(...page[1].map(String));
    } while (cursor !== '0' && versionKeys.length < 200);
    if (!versionKeys.length) continue;

    for (let start = 0; start < versionKeys.length; start += 50) {
      const batch = versionKeys.slice(start, start + 50);
      const removed = await redisCommand(['DEL', ...batch]);
      keysDeleted += Number(removed || 0);
    }
    versionsPruned += 1;
    bytesFreed += Number(row.stringBytes || 0);
  }
  return {
    versionsPruned,
    keysDeleted,
