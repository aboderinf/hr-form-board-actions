...
async function archiveEvaluation(predictions, through) {
  ...
  for (const row of predictions || []) {
    ...
    predictionMap.set(`${row.date}|${compactName(row.player || row.player_key)}`, row);
  }
...
function liveV2Rows(v1Rows, artifact, state, schedule, date) {
  ...
  for (const row of v1Rows || []) {
    ...
    output.push({
      ...
      v2Probability: Number(probability.toFixed(4)), v1Probability: row.modelProbability, formProbability: row.formProbability,
