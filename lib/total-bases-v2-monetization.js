...
function robustTuneStrategy(calibrationRows) {
  ...
  return best
    ? { ready: true, ...best, passingCandidates, candidatesTested: candidates.length, topCalibrationCandidates }
    : {
        ready: false,
        rule: null,
        score: null,
...
module.exports = async function totalBasesV2MonetizationHandler(request, response) {
  ...
  const cacheKey = `mlbtb2:monetization:v2:${date}:${String(request.query?.checkpoint || 'latest')}:${through}`;
  try {
