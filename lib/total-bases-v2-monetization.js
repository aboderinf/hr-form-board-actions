...
function recoveredSplit(calibrationRows) {
  const dates = [...new Set(calibrationRows.map((row) => row.date))].sort();
  if (dates.length < 2) return { splitDate: HOLDOUT_START, earlyDates: dates, lateDates: [] };
  const splitIndex = Math.max(1, Math.min(dates.length - 1, Math.floor(dates.length / 2)));
  const splitDate = dates[splitIndex];
  return {
    splitDate,
    earlyDates: dates.filter((date) => date < splitDate),
    lateDates: dates.filter((date) => date >= splitDate),
  };
}

function robustTuneStrategy(calibrationRows) {
  const split = recoveredSplit(calibrationRows);
  const earlyRows = calibrationRows.filter((row) => row.date < split.splitDate);
  const lateRows = calibrationRows.filter((row) => row.date >= split.splitDate);
  let best = null;
  ...
  for (const rule of candidates) {
    ...
    if (early.bets < 8 || early.slates < 2 || late.bets < 8 || late.slates < 2) continue;
    if (full.bets < 20 || full.slates < 6) continue;

    const earlyRoi = Number(early.roi || 0);
    const lateRoi = Number(late.roi || 0);
    const fullRoi = Number(full.roi || 0);
    const minimumHalfRoi = Math.min(earlyRoi, lateRoi);
    const imbalancePenalty = Math.abs(earlyRoi - lateRoi);
    const sampleBonus = Math.min(0.08, Math.log1p(full.bets) / 80);
    const score = minimumHalfRoi + 0.35 * fullRoi + sampleBonus - 0.20 * imbalancePenalty;
    diagnostics.push({ rule, score, early, late, full });

    if (!(earlyRoi > 0 && lateRoi > 0 && fullRoi > 0)) continue;
    if (early.profitableSlates < 1 || late.profitableSlates < 1 || full.profitableSlates < 3) continue;
  ...
  const topCalibrationCandidates = diagnostics.slice(0, 12);
  return best
    ? { ready: true, ...best, passingCandidates, candidatesTested: candidates.length, topCalibrationCandidates, split }
    : {
        ready: false,
        rule: null,
        score: null,
        early: strategySummary([]),
        late: strategySummary([]),
        full: strategySummary([]),
        passingCandidates,
        candidatesTested: candidates.length,
        topCalibrationCandidates,
        split,
      };
}
...
module.exports = async function totalBasesV2MonetizationHandler(request, response) {
  ...
  const cacheKey = `mlbtb2:monetization:v3:${date}:${String(request.query?.checkpoint || 'latest')}:${through}`;
  try {
  ...
  try {
    ...
    const output = {
      ...
      methodology: {
        ...
        tuning: `Candidate rules are evaluated only before ${HOLDOUT_START}. The calibration block is split at the median recovered slate date (${tuned.split.splitDate}) so archive gaps do not starve one side; both recovered-date halves must be profitable with minimum sample and no single-day profit concentration above 60%.`,
      ...
      split: {
        ...
        internalSplit: tuned.split.splitDate,
        earlyRecoveredDates: tuned.split.earlyDates,
        lateRecoveredDates: tuned.split.lateDates,
      ...
      calibration: {
        ...
        score: tuned.score,
        topCandidates: tuned.topCalibrationCandidates.map((candidate) => ({
          rule: { ...candidate.rule, label: ruleLabel(candidate.rule) },
