...
function startsBefore(gameLogs, slateDate) {
  return (gameLogs?.hitting || [])
    .filter((split) => String(split?.date || '') < slateDate)
    .filter((split) => Number(split?.stat?.plateAppearances || 0) > 0)
    .filter((split) => starterStatus(gameLogs, split?.date, splitGamePk(split), split) === true)
    .map((split) => {
...
module.exports = async function totalBasesDiscoveryHandler(request, response) {
  ...
  const cacheKey = `mlbtb2:discovery:v2:${start}:${requestedThrough}`;
  try {
    const cached = await redisCommand(['GET', cacheKey]);
    if (cached) {
    ...
    let nonStarterVoids = 0;
    let unverifiedStarterStatus = 0;
    ...
    for (const { capture, rows } of hydratedCaptures) {
      ...
      for (const oddsRow of rows) {
        ...
        const settled = settledBatterGame(gameLogs.get(Number(oddsRow.batterId)), capture.date);
        if (settled.status !== 'graded') {
          if (settled.status === 'ambiguous') ambiguousDoubleheaders += 1;
          else if (settled.reason === 'did_not_start') nonStarterVoids += 1;
          else if (settled.status === 'unverified') unverifiedStarterStatus += 1;
    ...
    const output = {
      ...
      methodology: {
        ...
        settlement: 'Official MLB hitting and fielding game logs. Pregame props are graded only when the hitter started and recorded a plate appearance; non-starters are void for FanDuel, DraftKings, and BetMGM. A player-date with more than one MLB appearance is skipped to avoid assigning a doubleheader result to the wrong archived prop.',
