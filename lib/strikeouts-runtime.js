...
const STRIKEOUT_KEYS = new Set(['pitchingstrikeouts']);
...
function isStrikeoutsOdd(odd) {
  if (!STRIKEOUT_KEYS.has(normalizeStat(odd?.statID))) return false;
...
function normalizeStrikeoutsProviderPayload(raw, slateDate, checkpoint, requestedAt, completedAt) {
  const rows = new Map();
  ...
  for (const event of raw?.data || []) {
    ...
    for (const [oddKey, oddValue] of Object.entries(event?.odds || {})) {
      const odd = oddValue || {};
      if (!isStrikeoutsOdd(odd)) continue;

      const providerEntityId = String(odd.statEntityID || '');
      const pitcherName = parsePlayerName(providerEntityId);
      if (!pitcherName) continue;
      const side = String(odd.sideID || '').toLowerCase();
      const rowKey = `${eventId}:${providerEntityId || playerKey(pitcherName)}`;
      ...
      if (!rows.has(rowKey)) {
        rows.set(rowKey, {
          ...
          pitcherId: null,
          pitcherName,
          playerKey: playerKey(pitcherName),
      ...
      for (const [bookName, bookValue] of Object.entries(odd?.byBookmaker || {})) {
        ...
        const line = Number(info.overUnder ?? odd.bookOverUnder ?? odd.fairOverUnder);
...
function strikeoutsArchiveKey(date, checkpoint) {
  return `mlbstrikeouts:checkpoint:${date}:${checkpoint}`;
}

async function repairStrikeoutsCheckpointFromRaw(date, checkpoint, existing = null) {
  const stored = await readRawArchive(date, checkpoint);
  if (!stored) return existing;
  const { archive, responsePayload } = stored;
  const rebuilt = normalizeStrikeoutsProviderPayload(
    responsePayload,
    date,
    checkpoint,
  ...
  if (archive.responseSha256 && rebuilt.providerResponseSha256 !== archive.responseSha256) {
    throw new Error('Raw provider archive SHA does not match rebuilt strikeouts checkpoint');
  }
  await redisCommand([
    'SET',
    strikeoutsArchiveKey(date, checkpoint),
...
async function readStrikeoutsCheckpoint(date, checkpoint) {
  const cp = normalizeCheckpoint(checkpoint);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(date || '')) || !cp) return null;
  const raw = await redisCommand(['GET', strikeoutsArchiveKey(date, cp)]);
  if (raw) {
    const payload = JSON.parse(raw);
    if (Number(payload.schemaVersion || 0) >= SCHEMA_VERSION) return payload;
  }
  return repairStrikeoutsCheckpointFromRaw(date, cp, raw ? JSON.parse(raw) : null);
}

module.exports = {
  BOOKS,
  normalizeStrikeoutsProviderPayload,
  readStrikeoutsCheckpoint,
  repairStrikeoutsCheckpointFromRaw,
  strikeoutsArchiveKey,
};
