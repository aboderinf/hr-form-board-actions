...
const TOP100_RUNTIME_URL = "https://hr-form-board-actions.vercel.app/api/top100-current";
...
async function loadTop100(slateDate) {
  try {
    const redisRaw = await redisCommand(["GET", `mlbhr:top100:${slateDate}`]);
    if (redisRaw) {
      const redisPayload = JSON.parse(redisRaw);
      if (redisPayload?.slate_date === slateDate && Array.isArray(redisPayload?.players)) {
        return redisPayload;
      }
    }
  } catch {
    // Continue to the deployment/static fallbacks. A read-path Redis error must
    // not make historical GitHub mirrors unusable.
  }

  const local = localTop100();
  if (local?.slate_date === slateDate && Array.isArray(local?.players)) return local;

  const response = await fetch(`${TOP100_RUNTIME_URL}?slate=${encodeURIComponent(slateDate)}&t=${Date.now()}`, {
    cache: "no-store",
    headers: { "User-Agent": "hr-form-discovery-projector/2.0" },
...
function duplicateTop100Keys(top100) {
...
function selectMarket(player, markets, ambiguousKeys = new Set()) {
  const key = playerKey(player?.player || "");
  const candidates = markets.get(key) || [];
  if (!candidates.length) return null;

  const requiresTeamMatch = ambiguousKeys.has(key) || candidates.length > 1;
  if (!requiresTeamMatch && candidates.length === 1) return candidates[0];

  const teamToken = TEAM_ABBREVIATIONS[Number(player?.team_id)];
  if (teamToken) {
    const matched = candidates.filter((row) => matchupTeams(row?.matchup).includes(teamToken));
    if (matched.length === 1) return matched[0];
  }

  // Never assign a duplicate-name market by guess. Missing odds are safer than
  // attaching another player's price to this Top 100 row.
  return null;
}
...
function buildDiscoveryArchive(top100, checkpoint) {
  ...
  const ambiguousKeys = duplicateTop100Keys(top100);
  const entries = [];
  let pricedRows = 0;
  let pregamePricedRows = 0;

  for (const player of top100.players || []) {
    const market = selectMarket(player, markets, ambiguousKeys);
...
async function ensureDiscoveryArchive(date, checkpoint) {
  ...
  if (existing && Number(existing.schema_version || 0) >= DISCOVERY_SCHEMA_VERSION) return existing;
  ...
  try {
  if (existing) {
    await redisCommand(["SET", key, JSON.stringify(archive), "EX", DISCOVERY_TTL_SECONDS]);
  } else {
    ...
    if (inserted !== "OK") {
      ...
      if (raced && Number(raced.schema_version || 0) >= DISCOVERY_SCHEMA_VERSION) return raced;
      await redisCommand(["SET", key, JSON.stringify(archive), "EX", DISCOVERY_TTL_SECONDS]);
    }
  }
  ...
  await redisCommand([
    ...
    // The source checkpoint and Top 100 data have already been verified.
    // A failed derived-cache write must not hide available historical data.
    console.warn('Discovery archive cache write deferred:', String(error.message || error));
  }
  return archive;
}
