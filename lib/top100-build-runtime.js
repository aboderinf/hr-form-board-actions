const { currentEtDate, redisCommand, releaseLease, writeDerivedCache } = require("./checkpoint-runtime");
...
async function refreshTop100() {
  ...
  const lock = await redisCommand(["SET", lockKey, owner, "NX", "EX", TOP100_LOCK_SECONDS]);
  if (lock !== "OK") {
  ...
  try {
    ...
    await writeDerivedCache(["ZADD", "mlbhr:top100-history", score, `mlbhr:top100:${slateDate}`]);
    return { outcome: "built", payload };
  } finally {
    try { await releaseLease(lockKey, owner); } catch { /* lock expires */ }
  }
}
