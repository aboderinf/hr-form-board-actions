...
const {
  ...
  settledBatterGame,
} = require('./total-bases-settlement');
...
async function persistSelectionSnapshot(snapshot) {
  ...
  try {
    ...
    console.warn('Frozen selection was not saved:', String(error.message || error));
    return null;
  }
}
...
async function bulkGameLogs(ids, season) {
  ...
  await Promise.all(groups.map(async (group) => {
    const params = new URLSearchParams({
      ...
      hydrate: GAME_LOG_HYDRATE(season),
      fields: GAME_LOG_FIELDS,
    });
    const payload = await fetchJson(`${MLB}/people?${params.toString()}`, 25000);
    for (const person of payload?.people || []) {
      const id = Number(person?.id);
      if (Number.isFinite(id)) output.set(id, extractBatterGameLogs(person));
    }
  }));
  return output;
}
...
function gradeSelection(selection, date, gameLogs) {
  ...
  const appearance = settledBatterGame(gameLogs.get(batterId), date);
  if (appearance.status === 'void') {
    return {
      ...selection,
      result: 'void',
      voidReason: appearance.reason,
  ...
  if (appearance.status === 'ambiguous' || appearance.status === 'unverified') {
    return {
      ...selection,
...
function summarizeGradedSelections(selections) {
  ...
  return {
    ...
    nonStarterVoids: selections.filter((row) => row.result === 'void' && row.voidReason === 'did_not_start').length,
    ambiguous: selections.filter((row) => row.result === 'ambiguous').length,
    unverified: selections.filter((row) => row.result === 'unverified').length,
...
function summarizeDays(days) {
  ...
  return {
    ...
    nonStarterVoids: usable.reduce((sum, day) => sum + Number(day.nonStarterVoids || 0), 0),
    unverified: usable.reduce((sum, day) => sum + Number(day.unverified || 0), 0),
    hitRate: bets ? Number((wins / bets).toFixed(4)) : null,
    netUnits: Number(netUnits.toFixed(3)),
    roi: bets ? Number((netUnits / bets).toFixed(4)) : null,
...
async function forwardPerformance(through) {
  const empty = {
    ...
    overallOutOfSample: {
      ...
      voids: 0, nonStarterVoids: 0, unverified: 0,
  ...
  const daily = snapshots.map(({ date, snapshot }) => {
    if (!snapshot) {
      return {
        ...
        nonStarterVoids: 0, ambiguous: 0, unverified: 0, netUnits: 0, roi: null, selections: [],
        cumulative: {
    ...
    const status = summary.ambiguous > 0 || summary.unverified > 0 ? 'partial' : 'settled';
    return {
      date,
      status,
      ...summary,
  ...
  return {
    ...
    overallOutOfSample: {
      ...
      nonStarterVoids: allForward.nonStarterVoids,
      unverified: allForward.unverified,
  ...
  try {
    ...
    const output = {
      ...
      methodology: {
        ...
        forwardTracking: 'Each day’s top-three list is snapshotted immutably at the 8:17 AM execution checkpoint. Official MLB hitting and fielding game logs settle each pregame prop only when the hitter started and recorded a plate appearance; non-starters are void, and outcomes never promote a replacement player.',
      },
      split: {
